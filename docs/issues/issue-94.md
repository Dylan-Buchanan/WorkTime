...
2. Issues 87–91 provide the OAuth flow, enumeration, sync, classification, and data access — but nothing renders them.
...
- **Connect**: OAuth connect button starts the authorize redirect (issue 87's flow); after the callback round-trip (issue 93) the card loads settings and shows the connected username.
...
- **Preview modal**: per-repo Sync runs through `classifyGithubIssues` (issue 90) with the repo's `project_id` seeding the default project, overridable per task; confirming creates tasks via the existing `createTask` PM callback; zero-issue/zero-new results show empty-state copy.
...
1. `src/components/GithubIntegrationCard.tsx` implementing the connect, repo list, per-repo controls, stale affordance, preview modal, and summaries described above, consuming `GitHubDataAccess` (issue 91) and the classification engine (issue 90).
...
- Depends on issues 87 (connect flow), 88 (labels/staleness payload), 89+90 (sync + classification), 91 (data access), and coordinates with 93 for the callback round-trip.
