...
describe("LocalStagingStore", () => {
    ...
    it("returns a fresh non-initialized default for an absent record", () => {
        ...
        expect(record.petNotableEvents).toEqual({});
        expect(record.petNotableEventUpdatedAt).toEqual({});
    ...
    it("defaults the pet notable-event group on fresh and legacy records", async () => {
        const store = new LocalStagingStore(window.localStorage);
        const fresh = store.read(OWNER_A);
        expect(fresh.petNotableEvents).toEqual({});
        expect(fresh.petNotableEventUpdatedAt).toEqual({});
        expect(fresh.petNotableEventTombstones).toEqual({});
        ...
        // A populated notable-event group round-trips through serialize then parse.
        await store.update(OWNER_A, (r) => ({
            ...r,
            petNotableEvents: {
                e1: {
            ...
            petNotableEventUpdatedAt: { e1: "2026-09-17T10:00:00.000Z" },
        }));
        const roundTripped = store.read(OWNER_A);
        expect(roundTripped.petNotableEvents.e1.title).toBe("First reliable sit");
    ...
    it("migrates v5 records to v7 with an empty local progress map", async () => {
        ...
        delete legacy.inProgressPomodoros;
        window.localStorage.setItem(key, JSON.stringify(legacy));
    ...
    it("rejects malformed local progress maps", async () => {
        const store = new LocalStagingStore(window.localStorage);
        await store.update(OWNER_A, (record) => ({ ...record, inProgressPomodoros: { t1: 0, t2: 600 } }));
        const key = stagingKey(OWNER_A);
        const valid = JSON.parse(window.localStorage.getItem(key) as string) as Record<string, unknown>;
        expect(store.read(OWNER_A).inProgressPomodoros).toEqual({ t1: 0, t2: 600 });

        for (const malformed of [[], "bad", { t1: -1 }, { t1: 0.5 }, { t1: null }]) {
            window.localStorage.setItem(key, JSON.stringify({ ...valid, inProgressPomodoros: malformed }));
            expect(() => store.read(OWNER_A)).toThrow(/inProgressPomodoros/);
        }
    });
    ...
    it("migrates an existing v2 owner record to empty to-do maps in memory", async () => {
        ...
        await seedInitialized(store, OWNER_A);
        const key = stagingKey(OWNER_A);
        const v2 = JSON.parse(window.localStorage.getItem(key) as string) as Record<string, unknown>;
        v2.schemaVersion = 2;
        delete v2.todos;
        delete v2.todoUpdatedAt;
        delete v2.todoTombstones;
        delete (v2.lastSynced as Record<string, unknown>).todos;
