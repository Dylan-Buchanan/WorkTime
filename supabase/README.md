...
`db reset` also replays `supabase/seed.sql`, which creates a fixed local test user (`dbuchananh@gmail.com` / `Test123!`). It is idempotent: re-runs update the password rather than duplicating the user, and it also rewrites the password of an existing `dbuchananh@gmail.com` so the credentials always match. Never run this seed against a hosted project.
