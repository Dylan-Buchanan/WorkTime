...
When finished implementing a feature run `pnpm version minor --no-git-tag-version`. When finished implementing a bug fic run `pnpm version patch --no-git-tag-version`. Major version upgrades should never be done.
